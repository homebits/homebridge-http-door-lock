...
import { HttpDoorLockPlatformAccessory } from './platformAccessory.js';
 ...
 */
export class HttpDoorLockPlatform implements DynamicPlatformPlugin
{
    ...
    discoverDevices()
    {
        ...
        for (const device of this.config.locks)
        {
            ...
            if (existingAccessory)
            {
                ...
                new HttpDoorLockPlatformAccessory(this, existingAccessory);

                const accessoriesToRemove = this.accessories.filter(accessory =>
                {
                    return !this.config.locks.some((device: { name: BinaryLike; }) => this.api.hap.uuid.generate(device.name) === accessory.UUID);
                });
            ...
            else
            {
                ...
                // this is imported from `platformAccessory.ts`
                new HttpDoorLockPlatformAccessory(this, accessory);
